...
const PIXELMART_ROUTE_SNIPPETS = {
  ...
  // Stage 8: SSRF — server fetches attacker-controlled URL
  7: [
    '// ============================================================',
    '// POST /shop/seller/import',
    '// VULNERABLE: fetches a user-supplied URL server-side, no validation',
    '// ============================================================',
    'router.post("/shop/seller/import", async (req, res) => {',
    '  const { url } = req.body;',
    '  if (!url) return res.status(400).json({ error: "url is required" });',
    '',
    '  // VULNERABLE: no check for private IP ranges!',
    '  // Fix: block 169.254.x.x, 10.x.x.x, 172.16.x.x, 127.x.x.x',
    '  const response = await fetch(url);',
    '  const text = await response.text();',
    '',
    '  res.json({ success: true, url, data: text });',
    '});',
  ].join('\n'),
...
const PIXELMART_NOTES = {
  ...
  7: [
    'TODO List (PixelMart admin):',
    '- Seller import endpoint fetches ANY URL the user provides, no domain validation!',
    '- Server is on AWS — 169.254.169.254 (instance metadata) is reachable internally',
    '- SSRF could leak IAM credentials, internal service data, VPC configuration',
    '- Fix: validate URL scheme (https only), block private IP ranges before fetching',
    '- Fix: use a domain allowlist for product data imports',
    '',
    'AWS metadata service (reachable from server): http://169.254.169.254',
    'IAM credentials path: /latest/meta-data/iam/security-credentials/<role-name>',
    'Import endpoint: POST /shop/seller/import (body: url=...)',
  ].join('\n'),
  8: [
    'TODO List (PixelMart admin):',
...
function buildFilesystem(stageIndex) {
  ...
  const pixelMartBashHistory = isPixelMartStage ? [
    ...
    'curl -X POST http://portal.megacorp.internal/shop/seller/import -d "url=http://169.254.169.254/latest/meta-data/"',
